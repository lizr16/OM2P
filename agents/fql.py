...
class FQLAgent(flax.struct.PyTreeNode):
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        elif(self.config['training_task'] == 4):
    ...
    def sample_actions(
        ...
        if(self.config['model_arch'] == 'score_net'):
            actions = self.network.select('actor_onestep_flow')(observations, noises, deterministic=deterministic, rngs={'dropout': dropout_key})
        else:
