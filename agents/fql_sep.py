...
class FQLAgent_sep(flax.struct.PyTreeNode):
    ...
    def softmax_q_loss(self, batch, grad_params, rng):
        ...
        if(self.config['model_arch'] == 'score_net'):
            actor_actions = self.network['actor'].select('actor_onestep_flow')(batch['observations'], noises, rngs={'dropout': dropout_rng}, params=grad_params)
        else:
            actor_actions = self.network['actor'].select('actor_onestep_flow')(batch['observations'], noises, params=grad_params)
    ...
    def actor_loss_scorenet(self, batch, grad_params, rng):
        ...
        pred = self.network['actor'].select('actor_bc_flow')(batch['observations'], x_t, t, rngs={'dropout': dropout_rng}, params=grad_params)
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        pred = self.network['actor'].select('actor_bc_flow')(batch['observations'], x_t, t, params=grad_params)
        bc_flow_loss = jnp.mean((pred - vel) ** 2)
    ...
    def update(self, batch):
        ...
        new_network_actor, actor_info = self.network['actor'].apply_loss_fn(loss_fn=loss_fn_actor)
    ...
    def sample_actions(
        ...
        if(self.config['model_arch'] == 'score_net'):
            actions = self.network['actor'].select('actor_onestep_flow')(observations, noises, deterministic=deterministic, rngs={'dropout': dropout_key})
        else:
            actions = self.network['actor'].select('actor_onestep_flow')(observations, noises)
    ...
    def sample_actions_multiq(
        ...
        if(self.config['model_arch'] == 'score_net'):
            actions = self.network['actor'].select('actor_onestep_flow')(observations_multiq_2d.T, noises.T, deterministic=deterministic, rngs={'dropout': dropout_key})
        else:
            actions = self.network['actor'].select('actor_onestep_flow')(observations_multiq_2d.T, noises.T)
        actions = jnp.clip(actions, -1, 1)
    ...
    def sample_flow_actions(
        ...
        for i in range(self.config['flow_steps']):
            ...
            if(self.config['model_arch'] == 'score_net'):
                vels = self.network['actor'].select('actor_bc_flow')(observations, actions, t, is_encoded=True, deterministic=deterministic, rngs={'dropout': dropout_key})
            else:
                vels = self.network['actor'].select('actor_bc_flow')(observations, actions, t, is_encoded=True, deterministic=deterministic)
    ...
    def create(
        ...
        rng, init_actor_rng, init_critic_rng = jax.random.split(rng, 3)
        ...
        actor_networks = {k: v[0] for k, v in actor_network_info.items()}
        actor_network_args = {k: v[1] for k, v in actor_network_info.items()}
        ...
        actor_lr = config['actor_lr']   # Learning rate for actor
        ...
        actor_network_def = ModuleDict(actor_networks)
        critic_network_def = ModuleDict(critic_networks)
        
        actor_network_params = actor_network_def.init(init_actor_rng, **actor_network_args)['params']
        ...
        actor_state = TrainState.create(actor_network_def, actor_network_params, tx=actor_tx)
